...
public:
   ...
   // Adicionar destrutor virtual para evitar comportamento indefinido
   virtual ~DVD() = default;
...
public:
   ...
   // Método para consultar por índice
   void consultar(int indice) {
      if (indice >= 0 && indice < numeroDVDs) {
         cout << "DVD encontrado no indice " << indice << ":" << endl;
         cout << "-----------------------------------" << endl;
         dvds[indice]->imprimir();
         cout << "-----------------------------------" << endl;
      } else {
