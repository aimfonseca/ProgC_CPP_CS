...
int main(void)
{
    ...
    Font font1 = LoadFont("C:/raylib/raylib/examples/text/resources/custom_mecha.png");          // Font loading
    ...
    Texture2D background = LoadTexture("VHS_is_DaBEST.png");
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        BeginDrawing();
            ...
            ClearBackground(RAYWHITE); // Or choose a background color as a fallback
            DrawTexture(background, 0, 0, WHITE); // Draw background first

            // Draw shadow text with slight offsets in black
            /* Vector2 shadowOffset = { 2, 2 }; // Offset for the shadow (adjust as needed)
            DrawTextEx(font1, msg1, (Vector2){ fontPosition1.x + shadowOffset.x, fontPosition1.y + shadowOffset.y }, 
                        (float)font1.baseSize, -3, BLACK);
*/
            // Draw multiple layers for a blur effect
            for (int i = -2; i <= 2; i++) {
                for (int j = -2; j <= 2; j++) {
                    if (i != 0 || j != 0) { // Avoid drawing the center text layer
                        DrawTextEx(font1, msg1, (Vector2){ fontPosition1.x + i, fontPosition1.y + j },
                                (float)font1.baseSize, -3, BLACK);
                    }
                }
            }

            DrawTextEx(font1, msg1, fontPosition1, (float)font1.baseSize, -3, WHITE); // Draw text on top
